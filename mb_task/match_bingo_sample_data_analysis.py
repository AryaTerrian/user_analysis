...
def calculate_avg_clv(df: pd.DataFrame) -> float:
    """Your original formula – average of (monthly × lifetime)."""
    return (df["monthly_revenue"] * df["lifetime_months"]).mean()
...
with c2:
    ...
    st.caption(
        "Average customer-lifetime value so far — "
...
st.caption(
        "The bar-chart below shows that spend rises sharply with games played—"
        "players in the top quartile deposit far more than the rest."
    )
